...
render::Model buildSteppedBlock(double width, double depth, double stepHeight, int divisions)
{
    render::Model model;
    ...
    for (int row = 0; row < samples; ++row)
    {
        for (int col = 0; col < samples; ++col)
        {
            ...
            const double z = (x < width * 0.5) ? 0.0 : -stepHeight;
...
int main()
{
    ...
    render::Model model = buildSteppedBlock(kWidth, kDepth, kStep, kDivisions);
    ...
    for (const tp::Polyline& poly : toolpath.passes)
    {
        ...
        {
            const double surfaceZ = (static_cast<double>(vertex.p.x) < kWidth * 0.5) ? 0.0 : -kStep;
